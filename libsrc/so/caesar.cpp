...
string encrCaesarText(const string& text, int shift, int m) {
    string result;
    for (unsigned char c : text) result.push_back(encrCaesarByte(shift, m, c));
    return result;
}

string decrCaesarText(const string& text, int shift, int m) {
    string result;
    for (unsigned char c : text) result.push_back(decrCaesarByte(shift, m, c));
    return result;
}
...
bool encrCaesarFile(const string& input, const string& output, int shift, int m) {
    try {
        ...
        char byte;
        while (in.get(byte)) out.put(encrCaesarByte(shift, m, (unsigned char)byte));
        return true;
    } catch (...) { return false; }
}
...
bool decrCaesarFile(const string& input, const string& output, int shift, int m) {
    try {
        ...
        if (!out.is_open()) return false;
        char byte;
        while (in.get(byte)) out.put(decrCaesarByte(shift, m, (unsigned char)byte));
        return true;
    } catch (...) { return false; }
}
