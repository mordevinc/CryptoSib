#include "../menu.h"
#include "../../../crossPlatform/crypto/crossCrypto.h"  // для strToHex, hexToStr

vector<int> Windows::checkKeyPermutation() {
    string input;
    vector<int> key;
...
void Windows::enterTextToEncryptPermutation(const vector<int>& key) {
    cout << "Введите текст для шифрования" << endl;
    string text = checkText();

    // Преобразуем в HEX перед перестановкой
...
void Windows::enterTextToDecryptPermutation(const vector<int>& key) {
...
void Windows::enterFileToEncryptPermutation(const vector<int>& key) {
...
void Windows::enterFileToDecryptPermutation(const vector<int>& key) {
...
void Windows::choiceFuncOfTextPermutation() {
    try {
...
void Windows::choiceFuncOfFilePermutation() {
    try {
        vector<int> key = checkKeyPermutation();
